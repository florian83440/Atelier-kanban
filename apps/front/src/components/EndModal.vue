...
function render() {
  ...
  const h = props.history || { labels: [], revenue: [], penalties: [], spending: [], staffUsage: [] };
  const datasets = [
  ...
  if ((h.spending || []).some((v) => v > 0)) {
    datasets.push({ label: 'Recrutement (€)', data: h.spending, borderColor: '#c47f00', borderDash: [6, 3], fill: false, yAxisID: 'y' });
  }
  datasets.push({ label: 'Ressources occupées', data: h.staffUsage, borderColor: '#0b74c4', borderDash: [4, 4], fill: false, yAxisID: 'y1' });
  chart = new Chart(canvas.value.getContext('2d'), {
    ...
    options: {
      ...
      scales: {
        ...
        y1: { type: 'linear', position: 'right', min: 0, suggestedMax: 12, grid: { drawOnChartArea: false }, ticks: { color: '#64748b' } },
